...
sub start_daemon {
    ...
    if ( !$pid ) {
        my $daemon = Algorithm::EventsPerSecond::Sukkal->new(
            socket => $sock_path, window => 300, %opts );
    ...
    for ( 1 .. 100 ) {
        my $c = IO::Socket::UNIX->new(
            Type => Socket::SOCK_STREAM(), Peer => $sock_path );
        if ($c) {
            print {$c} "PING\n";
            my $reply = <$c>;
...
{
    my $rate = $M->build(
        { munger => 'eps', socket => $sock, prefix => 't1:' } );
    my $r;
    ...
    my $rate_ro = $M->build( { munger => 'eps', socket => $sock,
        prefix => 't1:', mark => 0 } );
    ok( $rate_ro->('k') > 0, 'rate is positive once the meter has aged' );

    my $total = $M->build( { munger => 'eps', socket => $sock,
        prefix => 't1:', read => 'total', mark => 0 } );
    is( $total->('k'), 5, 'five marks landed (read-only total)' );

    my $count = $M->build( { munger => 'eps', socket => $sock,
        prefix => 't1:', read => 'count', mark => 0 } );
...
# ---- prefix isolation -------------------------------------------------------
{
    my $t2 = $M->build( { munger => 'eps', socket => $sock,
        prefix => 't2:', read => 'total', mark => 0 } );
    is( $t2->('k'), 0, 'same source value under another prefix is a fresh key' );
}

# ---- key sanitization -------------------------------------------------------
{
    my $mark = $M->build( { munger => 'eps', socket => $sock,
        prefix => 't3:', read => 'total' } );
    my $out = $mark->('has spaces');
    is( $out, 1, 'whitespace in the value is sanitized, mark lands' );

    my $ro = $M->build( { munger => 'eps', socket => $sock,
        prefix => 't3:', read => 'total', mark => 0 } );
...
# ---- multi-output: rate + count in one round trip ----------------------------
{
    my $plan = $M->compile(
        ...
        mungers => {
            req => { munger => 'eps', socket => $sock, prefix => 'm1:',
                from => 'ip', parts => [qw(rate count)],
...
# ---- on_error ----------------------------------------------------------------
{
    my $quiet = $M->build( { munger => 'eps', socket => "$dir/nope.sock",
        prefix => 'x:', on_error => 0 } );
    is( $quiet->('k'), 0, 'unreachable daemon with numeric on_error falls back' );

    my $loud = $M->build( { munger => 'eps', socket => "$dir/nope.sock",
        prefix => 'x:' } );
    eval { $loud->('k') };
    ...
    my $plan = $M->compile(
        tags    => [qw(a b)],
        mungers => { m => { munger => 'eps', socket => "$dir/nope.sock",
            from => 'ip', on_error => -1,
            parts => [qw(rate count)], into => [qw(a b)] } },
    );
    is_deeply( $plan->apply_named( { ip => 'k' } ), [ -1, -1 ],
        'multi-output on_error fills all columns' );
}
...
SKIP: {
    ...
    eval { $rate->('second') };
    like( $@, qr/iqbi-damiq replied: ERR key limit/,
        'MARKRATE key-limit error croaks with the daemon message' );
    ...
    my $quiet = $M->build( { munger => 'eps', socket => $sock2,
        prefix => 'L:', on_error => -1 } );
...
{
    ok( $M->build( { munger => 'eps', socket => "$dir/nothere.sock" } ),
        'building an eps munger never connects' );
    ...
    like( $@, qr/'parts' is for the multi-output form/,
        'scalar eps rejects parts without into' );
    ...
    eval {
        $M->compile( tags => [qw(a)], mungers => { m => { munger => 'eps',
            from => 'ip', parts => ['bogus'], into => ['a'] } } );
    };
    like( $@, qr/unknown part 'bogus'/, 'bad multi part croaks at compile' );

    # eager validation in write_info works with no daemon running
    require Algorithm::Classifier::IsolationForest::Zorita;
    my $z = Algorithm::Classifier::IsolationForest::Zorita->new(
        basedir => tempdir( CLEANUP => 1 ) );
    my $path = $z->write_info( slug => 's', set => 'x', info => {
        tags    => [qw(req_rate)],
        mungers => { req_rate => { munger => 'eps', from => 'src_ip',
            socket => "$dir/nothere.sock", prefix => 'r:' } } } );
    ok( -f $path, 'write_info validates an eps set without a daemon' );
}
