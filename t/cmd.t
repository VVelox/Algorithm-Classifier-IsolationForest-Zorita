...
sub make_set {
    ...
    my $writer = Algorithm::Classifier::IsolationForest::Zorita::Writer->new(
        zorita => $zorita, slug => $slug, set => $set, writer => 'w01' );
...
# ---------------------------------------------------------------------------
{
    ...
    ok( !model_exists( $zorita, 'appone', 'ssh-logs' ),
        'rebuild touched only the named set' );
}
...
# ---------------------------------------------------------------------------
{
    ...
    ok( !model_exists( $zorita, 'apptwo', 'web-logs' ),
        'a different slug was left alone' );
}
...
# run still exits non-zero because one set failed.
{
    ...
    ok( model_exists( $zorita, 'apptwo', 'web-logs' ),
        'the good set built despite its sibling failing' );
}
...
# ---------------------------------------------------------------------------
{
    my ( $base, $zorita ) = fresh_tree();
    my $r = test_app( $APP,
        [ '--basedir', $base, 'rebuild', '--hours', '48', 'appone', 'http-logs' ] );
    is( $r->exit_code, 0, 'rebuild --hours exits 0' );
    ok( model_exists( $zorita, 'appone', 'http-logs' ),
        'rebuild --hours still renders the model' );
}
