...
{
	my $writer = new_writer( mungers => { method => { munger => 'enum', map => { GET => 0, POST => 1 } } }, );
	...
	is_deeply( [ @lines[ 1 .. 3 ] ], [ '100,0,200', '250,1,201', '999,0,404' ], 'batch rows munged and in order', );
...
# ---- mixed hashref / arrayref records ---------------------------------------
{
	...
	my $file   = $writer->write_rows( [ { bytes => 1, method => 2, status => 3 }, [ 4, 5, 6 ], ] );
	my @lines  = slurp_lines($file);
	is_deeply( [ @lines[ 1, 2 ] ], [ '1,2,3', '4,5,6' ], 'named and positional records mix in one batch' );
}
...
# ---- all-or-nothing: a bad record aborts the whole batch --------------------
{
	...
	eval { $writer->write_rows( [ [ 7, 8, 9 ], [ 10, 'not numeric', 12 ], ] ); };
	like( $@, qr/row 1: field 1 .* is not clean numeric data/, 'bad record croaks naming the row and field' );
	is( scalar( slurp_lines($file) ), 2, 'nothing from the failed batch was appended' );

	eval { $writer->write_rows( [ [ 1, 2 ] ] ) };
	like( $@, qr/row has 2 fields/, 'arity is checked per record' );

	eval { $writer->write_rows( ['scalar'] ) };
	like( $@, qr/each record must be a hashref or an arrayref/, 'non-ref record rejected' );
