...

{
    ...
    like( $file10, qr{/2026-07-06/10/w\.web01\.csv\z},
        'first write lands in the 10:00 hour directory' );
    is( $file10b, $file10,
        'a later write in the same hour reuses the cached path' );
    like( $file11, qr{/2026-07-06/11/w\.web01\.csv\z},
        'a write in the next hour rolls onto a new directory' );
    isnt( $file11, $file10, 'the new hour is a different file' );

    # each hour file carries its own single header + only its own rows
    is_deeply( [ slurp_lines($file10) ],
        [ 'bytes,duration,status', '1,0.1,200', '2,0.2,200' ],
        'hour 10 file has one header and both of its rows' );
    is_deeply( [ slurp_lines($file11) ],
