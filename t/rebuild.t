...
# ----------------------------------------------------------------------------
{
    ...
    is_deeply( $if->feature_names, [@TAGS],
        'tags forwarded as feature_names' );
    is( $if->decision_threshold, undef,
        'unfitted model has no decision threshold yet' );
...
# ----------------------------------------------------------------------------
{
    ...
    ok( !-f $zorita->model_path( slug => 'myapp', set => 'http-logs' ),
        'no model file before rebuild' );

    my $model = $zorita->rebuild_model(
        slug => 'myapp', set => 'http-logs', time => $NOW );

    isa_ok( $model, 'Algorithm::Classifier::IsolationForest',
        'rebuild_model() returns a fitted model' );
    ok( defined $model->decision_threshold,
        'fitted model learned a decision threshold from contamination' );
    ok( -f $zorita->model_path( slug => 'myapp', set => 'http-logs' ),
        'iforest_model.json written to disk' );
...
# rebuild_model() has nothing to train on when the window is empty.
{
    ...
    like( $@, qr/no training data in window/,
        'rebuild_model() croaks on an empty window' );
}
...
# ----------------------------------------------------------------------------
{
    ...
    is_deeply( $loaded->feature_names, [@TAGS],
        'loaded model preserves feature_names' );
    is( $loaded->decision_threshold, $built->decision_threshold,
        'loaded model preserves the decision threshold' );
    ...
    my $rows = $zorita->read_back( slug => 'myapp', set => 'http-logs', time => $NOW );
    is_deeply( $loaded->predict($rows), $built->predict($rows),
