...
# ----------------------------------------------------------------------------
{
    ...
    for my $field (qw(slug set writer)) {
        my %args = ( zorita => $zorita, slug => 'myapp',
            set => 'http-logs', writer => 'web01' );
        delete $args{$field};
...
# ----------------------------------------------------------------------------
{
    ...
    is_deeply( \@lines, [ 'bytes,duration,status', '1234,0.08,200' ],
        'first write emits header then the row' );

    my $file2 = $writer->write( [ 5678, 0.5, 404 ] );
    is( $file2, $file, 'second write targets the same hour file' );

    @lines = slurp_lines($file);
    is( scalar( grep { $_ eq 'bytes,duration,status' } @lines ), 1,
    ...
    like( $@, qr/row has 2 fields but info\.json declares 3/,
        'write() croaks when the row is too short' );
    ...
    for my $case ( sort keys %bad ) {
        ...
        like( $@, qr/is not clean numeric data/,
            "write() rejects $case" );
    }
...
# ----------------------------------------------------------------------------
{
    ...
    my $file = $writer->write_named(
        { status => 200, bytes => 1234, duration => 0.08 } );

    my @lines = slurp_lines($file);
    is( $lines[-1], '1234,0.08,200',
    ...
    like( $@, qr/requires a hashref/,
        'write_named() croaks on a non-hashref' );
}
