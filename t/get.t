...
# ---------------------------------------------------------------------------
{
	...
	is(
		$json,
		$z->info_json( slug => 'myapp', set => 'http-logs' ),
		'template and the set it created print identical JSON'
	);

	eval { $z->template_json( template => 'nope' ) };
	like( $@, qr/no template 'nope'/, 'template_json croaks on an unknown template' );
}
...
SKIP: {
	...
	is( $gt->stdout,    $z->template_json( template => 'http' ), 'get-template prints the template JSON verbatim' );
