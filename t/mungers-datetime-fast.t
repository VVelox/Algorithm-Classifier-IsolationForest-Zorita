...
for my $part (@PARTS) {
    ...
    for my $stamp (@STAMPS) {
        ...
        my %exp = (
            ...
            frac_week => ( $t->day_of_week * 86400 + $t->hour * 3600
                    + $t->min * 60 + $t->sec ) / 604800,
        );
        my $pi = 2 * atan2( 0, -1 );
...
# ---- memo: repeat hits and non-stale updates --------------------------------
{
    ...
    my ( $code, $arity ) = do {
        ...
        my $plan = $M->compile(
            tags    => [qw(s c)],
            mungers => { tw => { munger => 'datetime', from => 'ts', format => $FMT,
                parts => [qw(sin_week cos_week)], into => [qw(s c)] } },
        );
        ( $plan, 2 );
    };
...
# ---- non-fast formats still work via strptime -------------------------------
{
    ...
    my $c = $M->build( { munger => 'datetime',
        format => '%d/%b/%Y:%H:%M:%S', part => 'hour' } );
