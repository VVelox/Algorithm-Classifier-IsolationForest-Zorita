...
# ---------------------------------------------------------------------------
{
    ...
    ok( !$z->valid_name('.set_templates'),
        'a leading-dot name is not a valid name' );
...
# ---------------------------------------------------------------------------
{
    ...
    is( $z->template_dir, File::Spec->catdir( $base, $TEMPLATE_DIR ),
        'template_dir is .set_templates under basedir' );
    is( $z->template_path( template => 'http' ),
...
# ---------------------------------------------------------------------------
{
    ...
    is_deeply( [ $z->templates ], [],
        'templates() is empty before any template dir exists' );

    my $path = $z->write_template( template => 'http', info => {%HTTP} );
    ok( -f $path, 'write_template created the file' );
    is( $path, $z->template_path( template => 'http' ),
        'write_template returns the template path' );

    $z->write_template( template => 'ssh', info => { tags => [qw(a b)] } );

    is_deeply( [ $z->templates ], [qw(http ssh)],
        'templates() lists names sorted, extension stripped' );

    is_deeply( $z->read_template( template => 'http' ), {%HTTP},
        'read_template round-trips the JSON body' );
    ...
    close $dh;
    is_deeply( [ $z->templates ], [qw(http ssh)],
        'templates() ignores non-.json and leading-dot files' );
}
...
# ---------------------------------------------------------------------------
{
    ...
    ok( !defined $z->read_info( slug => 'myapp', set => 'http-logs' ),
        'set does not exist before create_set' );

    my $info_path = $z->create_set(
        slug => 'myapp', set => 'http-logs', template => 'http' );

    is( $info_path, $z->info_path( slug => 'myapp', set => 'http-logs' ),
        'create_set returns the written info.json path' );
    is_deeply( $z->read_info( slug => 'myapp', set => 'http-logs' ), {%HTTP},
        'the set info.json is a copy of the template' );
    ...
    # unknown template
    eval {
        $z->create_set( slug => 'myapp', set => 'other', template => 'nope' );
    };
    ...
    unlike( join( ',', $z->slugs ), qr/\Q$TEMPLATE_DIR\E/,
        'slugs() does not report the .set_templates control dir' );
}
...
SKIP: {
    ...
    my $c = test_app( $APP,
        [ '--basedir', $base, 'create-set', 'myapp', 'http-logs', 'http' ] );
    is( $c->exit_code, 0, 'create-set exits 0' );
    like( $c->stdout, qr{created myapp/http-logs from template 'http'},
        'create-set reports what it made' );
    is_deeply( $z->read_info( slug => 'myapp', set => 'http-logs' ), {%HTTP},
        'create-set wrote the templated info.json' );

    # create-set: unknown template fails non-zero
    my $cbad = test_app( $APP,
        [ '--basedir', $base, 'create-set', 'myapp', 'x', 'nope' ] );
    ...
    my $carity = test_app( $APP,
        [ '--basedir', $base, 'create-set', 'myapp', 'http-logs' ] );
    isnt( $carity->exit_code, 0, 'create-set with two args fails' );
    like( $carity->error, qr/<slug> <set> <template>/,
        'create-set arity error explained' );
}
