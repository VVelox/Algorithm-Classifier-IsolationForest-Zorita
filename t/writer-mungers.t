...
# ---- write_named munges raw input, expander fills both time columns ---------
{
    ...
    is( $lines[-1], '0,1,0,1,404',
        'row: sin/cos pair, log1p(0), enum(POST), raw status' );
}
...
# ---- a munger croak surfaces as a write failure ----------------------------
{
    ...
    eval {
        $writer->write_named( {
            ...
            bytes     => 0, method => 'HEAD', status => 200,   # HEAD unmapped
        } );
    };
    like( $@, qr/enum munger.*no mapping for 'HEAD'/,
        'an enum miss croaks the write' );
}
...
# ---- positional write is refused when the set has an expander ---------------
{
    ...
    like( $@, qr/expanding mungers/,
        'positional write() rejected for a set with a sin/cos expander' );
}
...
# ---- a set with no mungers is unchanged (regression) -----------------------
{
    ...
    my $file   = $writer->write_named(
        { time_sin => 0, time_cos => 1, bytes => 1234, method => 2, status => 200 } );
    my @lines = slurp_lines($file);
...
# ---- eager validation: a bad munger config croaks at write_info -------------
{
    ...
    # unknown munger name
    eval {
        $zorita->write_info( slug => 'myapp', set => 'bad', info => {
            tags => [qw(a)], mungers => { a => { munger => 'bogus' } } } );
    };
    like( $@, qr/unknown munger 'bogus'/, 'write_info rejects an unknown munger' );
    ok( !-f $zorita->info_path( slug => 'myapp', set => 'bad' ),
        'nothing was written for the rejected set' );

    # broken expander coverage
    eval {
        $zorita->write_info( slug => 'myapp', set => 'bad2', info => {
            tags    => [qw(s c)],
            mungers => { tw => { munger => 'datetime', from => 'ts',
                format => $FMT, parts => [qw(sin_week cos_week)],
                into   => [qw(s nope)] } } } );
    };
    ...
    eval {
        $zorita->write_info( slug => 'myapp', set => 'bad3', info => {
            mungers => { a => { munger => 'log' } } } );
    };
    like( $@, qr/requires a non-empty 'tags'/,
        'write_info rejects mungers without tags' );

    # a valid munger config still writes
    my $path = $zorita->write_info( slug => 'myapp', set => 'good', info => {
        tags => [qw(a)], mungers => { a => { munger => 'log', offset => 1 } } } );
    ...
    eval {
        $zorita->write_info( slug => 'myapp', set => 'bad4',
            info => { tags => [ 'ok', 'not ok' ] } );
    };
    like( $@, qr/invalid tag 'not ok'/,
        'write_info rejects a tag violating the name rule' );
    eval {
        $zorita->write_info( slug => 'myapp', set => 'bad4',
            info => { tags => ['dotted.name'] } );
    };
    like( $@, qr/invalid tag 'dotted\.name'/,
        'write_info rejects a dotted tag' );
    eval {
        $zorita->write_info( slug => 'myapp', set => 'bad5',
            info => { tags => [qw(dup dup)] } );
    };
    ...
    eval {
        $zorita->write_template( template => 'broken', info => {
            tags => [qw(a)], mungers => { a => { munger => 'bogus' } } } );
    };
    like( $@, qr/unknown munger 'bogus'/,
        'write_template rejects a bad munger config' );
    ok( !-f $zorita->template_path( template => 'broken' ),
        'the rejected template was not written' );

    eval {
        $zorita->write_template( template => 'diepolicy',
            info => { tags => [qw(a)], missing => 'die' } );
    };
    like( $@, qr/may not be 'die'/, "write_template rejects missing => 'die'" );

    my $tpl = $zorita->write_template( template => 'good', info => {
        tags => [qw(a)], mungers => { a => { munger => 'log', offset => 1 } } } );
    ...
    close $fh;
    eval {
        $zorita->create_set( slug => 'myapp', set => 'from-tpl',
            template => 'dropped' );
    };
    like( $@, qr/unknown munger 'bogus'/,
        'create_set rejects a hand-dropped template with a bad munger config' );
}
